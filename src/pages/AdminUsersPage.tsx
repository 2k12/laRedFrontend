...
import { UserCheck, Users, Search, Loader2 } from "lucide-react";
...
export default function AdminUsersPage() {
    ...
    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            const currentFiltered = filteredUsers.filter(u => u.status === 'INACTIVE').map(u => u.id);
    ...
    const handleActivate = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/admin/users/activate`, {
                method: 'POST',
                headers: {
            ...
            if (res.ok) {
                toast.success(`${selectedIds.length} usuarios activados`);
                setSelectedIds([]);
                fetchUsers();
            } else {
                const data = await res.json();
                toast.error(data.error || "Error al activar usuarios");
            }
        } catch (error) {
            toast.error("Error de conexión");
        } finally {
    ...
    return (
        <div className="container mx-auto px-4 max-w-7xl pb-20">
            <PageHeader
                ...
                description="Activa y administra identidades del sistema"
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                    <div className="relative flex-1 sm:w-64">
                        ...
                        <input
                            ...
                            className="w-full bg-zinc-900/50 border border-white/5 rounded-xl pl-10 pr-4 h-11 text-sm focus:border-primary/50 transition-all"
                            value={searchQuery}
                    ...
                    {selectedIds.length > 0 && (
                        <Button
                            ...
                            className="h-11 bg-primary hover:bg-primary/90 text-primary-foreground font-black uppercase tracking-tighter italic"
                        >
                            {isActivating ? <Loader2 className="animate-spin w-4 h-4 mr-2" /> : <UserCheck className="w-4 h-4 mr-2" />}
                    ...
                    {loading ? (
                        ...
                        <div className="overflow-x-auto">
                            <Table>
                                <TableHeader className="bg-white/[0.02]">
                                    <TableRow className="border-white/5 hover:bg-transparent">
                                        <TableHead className="w-12 text-center">
                                            <input
                                                type="checkbox"
                                                className="rounded border-zinc-800 bg-zinc-900 checked:bg-primary"
                                                checked={selectedIds.length > 0 && selectedIds.length === inactiveCount}
                                ...
                                <TableBody>
                                    {filteredUsers.map((u) => (
                                        <TableRow key={u.id} className="border-white/5 hover:bg-white/[0.01] transition-colors group">
