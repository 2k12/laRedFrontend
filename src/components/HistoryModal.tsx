...
import { X, History, TrendingUp, TrendingDown, Clock, ShoppingBag, ArrowLeft } from "lucide-react";
                ...
                <>
                    ...
                    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-0 md:p-6 overflow-hidden">
                        ...
                        <motion.div
                            ...
                            className={cn(
                                "relative z-[100000] w-full overflow-hidden flex flex-col bg-zinc-950 shadow-2xl",
                                "h-[100dvh] md:h-[80vh] md:max-w-xl md:border md:border-white/10 md:rounded-[2.5rem]"
                            )}
                        >
                            {/* Header */}
                            <div className="flex items-center justify-between p-4 pt-[max(1rem,env(safe-area-inset-top))] md:p-8 border-b border-white/5 bg-zinc-950/50 backdrop-blur-xl absolute top-0 left-0 right-0 z-10 md:relative">
                                <div className="flex items-center gap-4">
                                    {/* Mobile Back Button */}
                                    <button
                                        onClick={onClose}
                                        className="md:hidden w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white active:scale-90 transition-transform"
                                    >
                                    ...
                                    <div>
                                        <h2 className="text-lg md:text-xl font-bold text-white tracking-tight italic uppercase">Historial Maestro</h2>
                                        <p className="text-[10px] text-zinc-500 font-black tracking-[0.2em] uppercase">Registro de Operaciones</p>
                                    </div>
                                </div>
                                <button
                                    onClick={onClose}
                                    className="hidden md:flex w-10 h-10 rounded-full hover:bg-white/5 items-center justify-center transition-colors text-zinc-500 hover:text-white"
                                >
                                    <X className="w-5 h-5" />
                                </button>
                            </div>
                            ...
                            {/* Content */}
                            <div className="flex-1 overflow-y-auto pt-28 pb-24 md:py-6 px-4 md:px-6 space-y-3 custom-scrollbar">
                                {loading ? (
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <div className="flex items-center gap-2">
                                                        <h4 className="text-sm font-bold text-white group-hover:text-primary transition-colors truncate max-w-[120px] sm:max-w-[150px] md:max-w-xs">{item.product_name}</h4>
                                                        <span className={cn(
                                                    ...
                                                    <p className="text-[10px] text-zinc-500 font-mono italic truncate max-w-[150px]">
                                                        {item.type === 'purchase' ? `V: ${item.store_name}` : `C: ${item.buyer_name}`}
                                                    </p>
                                                </div>
                                            </div>
                                            <div className="text-right shrink-0">
                                                <p className={cn(
                                                    ...
                                                    {item.type === 'purchase' ? '-' : '+'}{item.price_paid}
                                                </p>
                            ...
                            {/* Footer (Stats) */}
                            <div className="p-6 md:p-8 bg-black/40 border-t border-white/5 grid grid-cols-2 gap-4 pb-10 md:pb-8">
                                <div className="text-center">
                                    <p className="text-[8px] text-zinc-600 font-black uppercase tracking-widest mb-1">Compras</p>
                                    <p className="text-xl font-bold text-white tracking-widest">{history.filter(h => h.type === 'purchase').length}</p>
                                </div>
                                <div className="text-center border-l border-white/5">
                                    <p className="text-[8px] text-zinc-600 font-black uppercase tracking-widest mb-1">Ventas</p>
